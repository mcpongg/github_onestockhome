    ...
    fetch(gitUrl + user)
      ...
      .then(data => {
        ...
        fetch(data.followers_url)
          .then(response => response.json())
          .then(data => {
            this.setState({ followers: data.slice(0, 4) });

            //if condition
            if (this.state.followers.length != 0) {
              for (var i = 0; i < this.state.followers.length; i++) {
                var followerName = this.state.followers[i].login;

                this.setState({
                  followers_info: {
                    fol_name: this.state.followers[i].login,
                    fol_img: this.state.followers[i].avatar_url,
                    fol_res: this.state.followers[i].repos_url.length
                  }
                });

                console.log(this.state.followers_info);
    ...
    return (
      <div>
        <section class="margin">
          <input placeholder="Enter usename" type="text" ref="name" />

          <button onClick={() => this.handlerInput(this.refs.name.value)}>
            OK
          </button>
        </section>
        <div class="position2">
        ...
        <div class="position4">
          <td>{this.state.fols.map(fol => <h4>{fol.fol_res} </h4>)}</td>
        </div>
